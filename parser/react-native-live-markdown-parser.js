...
(() => {
  ...
  var HtmlEntities = __toESM(require_lib());
  ...
  var MARKDOWN_LINK_REGEX = new RegExp(`\\[([^\\][]*(?:\\[[^\\][]*][^\\][]*)*)]\\(${MARKDOWN_URL_REGEX}\\)(?![^<]*(<\\/pre>|<\\/code>))`, "gi");
  var MARKDOWN_IMAGE_REGEX = new RegExp(`\\!(?:\\[([^\\][]*(?:\\[[^\\][]*][^\\][]*)*)])?\\(${MARKDOWN_URL_REGEX}\\)(?![^<]*(<\\/pre>|<\\/code>))`, "gi");
    ...
    initializer() {
      this.rules = [
         ...
         */
        {
          ...
          replacement: (match, g1, g2) => `<img src="${str_default.sanitizeURL(g2)}"${g1 ? ` alt="${this.escapeAttributeContent(g1)}"` : ""} />`,
          rawInputReplacement: (match, g1, g2) => `<img src="${str_default.sanitizeURL(g2)}"${g1 ? ` alt="${this.escapeAttributeContent(g1)}"` : ""} data-raw-href="${g2}" data-link-variant="${typeof g1 === "string" ? "labeled" : "auto"}" />`
        },
        /**
         ...
         */
        {
          ...
          process: (textToProcess, replacement) => {
            const regex = new RegExp(`(?![^<]*>|[^<>]*<\\/(?!h1>))([_*~]*?)${MARKDOWN_URL_REGEX}\\1(?!((?:(?!<a).)+)?<\\/a>|[^<]*(<\\/pre>|<\\/code>|.+\\/>))`, "gi");
      ...
      this.htmlToMarkdownRules = [
        ...
        },
        {
          ...
          replacement: (match, g1, g2) => {
            ...
            resultString = map(resultString, prependGreaterSign).join("\n");
...
\`\`\``
      ...
      this.filterRules = (rule) => !contains(this.whitespaceRulesToDisable, rule.name);
      this.shouldKeepWhitespaceRules = filter(this.rules, this.filterRules);
      this.maxQuoteDepth = 3;
      this.currentQuoteDepth = 0;
    },
    getHtmlRuleset(filterRules, disabledRules, shouldKeepRawInput) {
      let rules = this.rules;
      const hasRuleName = (rule) => contains(filterRules, rule.name);
      const hasDisabledRuleName = (rule) => !contains(disabledRules, rule.name);
      if (shouldKeepRawInput) {
        rules = this.shouldKeepWhitespaceRules;
      }
      if (!isEmpty(filterRules)) {
        rules = filter(this.rules, hasRuleName);
      }
      if (!isEmpty(disabledRules)) {
        rules = filter(rules, hasDisabledRuleName);
      }
      return rules;
    },
    /**
    ...
    replace(text, { filterRules = [], shouldEscapeText = true, shouldKeepRawInput = false, disabledRules = [] } = {}) {
      let replacedText = shouldEscapeText ? escape_default(text) : text;
      ...
      try {
        ...
        const res = shouldEscapeText ? escape_default(text) : text;
        return res;
      }
      return replacedText;
    },
    ...
    replaceBlockElementWithNewLine(htmlString) {
      ...
      const stripHTML = (text) => str_default.stripHTML(text);
      splitText = map(splitText, stripHTML);
      let joinedText = "";
...
`;
    ...
    formatTextForQuote(regex, textToCheck, replacement) {
      if (textToCheck.match(regex)) {
        ...
        let textToFormat = map(textToCheck.split("\n"), formatRow).join("\n");
    ...
    extractLinksInMarkdownComment(comment) {
      try {
        ...
        const sanitizeMatch = (match) => str_default.sanitizeURL(match[1]);
        const links = map(matches, sanitizeMatch);
        return links;
      } catch (e) {
    ...
    getRemovedMarkdownLinks(oldComment, newComment) {
      ...
      return linksInOld === void 0 || linksInNew === void 0 ? [] : difference_default(linksInOld, linksInNew);
    },
    /**
    ...
    escapeAttributeContent(content) {
      ...
      originalContent = str_default.replaceAll(originalContent, "\n", "");
      return escape_default(originalContent);
    }
  };
    ...
    function dfs(node) {
      if (typeof node === "string") {
        ...
        if (node.tag === "<>") {
          ...
          const src = node.tag.match(/src="([^"]*)"/)[1];
          ...
          const linkString = rawLink ? index_default_default.unescape(rawLink[1]) : src;
          appendSyntax("!");
          if (hasAlt) {
            appendSyntax("[");
            processChildren(index_default_default.unescape(alt?.[1] || ""));
            appendSyntax("]");
          }
          appendSyntax("(");
